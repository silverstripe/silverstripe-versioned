        ...
        return DBDatetime::withFixedNow($now, function () {
            ...
            foreach ($liveOnlyKeys as $key) {
                if (array_key_exists($key, $liveObjs)) {
                    /** @var DataObject|Versioned $liveObj */
                    $liveObj = $liveObjs[$key];
                    $liveObj->doUnpublish();
                    // $liveObj->suppressDeletedVersion(function () use ($liveObj) {
                    //     $liveObj->doUnpublish();
                    // });
                    // $liveObj->deleteFromStage(Versioned::LIVE);
                    // $liveObj->suppressDeletedVersion(function () use ($liveObj) {
                    //     $liveObj->deleteFromStage(Versioned::LIVE);
                    // });
                    // $liveObj->isOnLiveOnly
                }
            }

            // Recursively publish owned DataObjects, starting with most nested ones
            foreach (array_reverse($draftObjs) as $obj) {
                $obj->publishSingle();
            }

            // Finally, publish this object
