...
class RecursivePublishable extends Extension
{
    ...
    public function publishRecursive()
    {
        ...
        return DBDatetime::withFixedNow($now, function () {
            ...
            // Keep the inferred ChangeSet name within the Varchar(255) limit on ChangeSet.Name
            $title = (string) ($owner->Title ?? '');
            $created = DBDatetime::now()->Nice();
            $translationKey = __CLASS__ . '.INFERRED_TITLE';
            $translationDefault = "Generated by publish of '{title}' at {created}";
            $translationFormat = _t($translationKey, $translationDefault);

            $titleLengthLimit = 255 - mb_strlen(strtr($translationFormat, [
                '{title}' => '',
                '{created}' => $created,
            ]));
            if ($titleLengthLimit > 0 && mb_strlen($title) > $titleLengthLimit) {
                $title = mb_substr($title, 0, $titleLengthLimit - 1) . '…';
            }

            $changeset->Name = strtr($translationFormat, [
                '{title}' => $title,
                '{created}' => $created,
            ]);
